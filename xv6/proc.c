...
// This is the heart of the syscall implementattion. Here, we create new kernel
// thread that shares the calling process's address space. Returns the PID of the
...
int clone(void(*fcn)(void*), void *arg, void *stack)
{
  ...
  // Allocate a new unused process in the process table.
  if((np = allocproc()) == 0){
    return -1;
  }
  ...
  *np->tf = *curproc->tf;
  ...
  void *stackArg, *stackRet;          //make some stack variables
  ...
  *(uint *)stackArg = (uint)arg;
  ...
  // Set the stack pointer register to base pointer and set instruction pointer
  np->tf->esp = (int)stack;
  memmove((void *)np->tf->esp, stack, PGSIZE);
  np->tf->esp += PGSIZE - 2 * sizeof(void *);
  np->tf->ebp = np->tf->esp;
  np->tf->eip = (int)fcn;
  //copy file descriptor from calling process
  for(i = 0; i < NOFILE; i++){
      if(curproc->ofile[i]){
        np->ofile[i] = filedup(curproc->ofile[i]);
      }
...
int join(int pid)
{
  ...
  for(;;){
    ...
    if (!ownChildthreads || curproc->killed) {
      release(&ptable.lock);
      return -1;
    }
